...
import com.mobtecnica.wafiapps.R;import com.mobtecnica.wafiapps.manager.HomeManager;
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().matches(HomeManager.BROADCAST_TOPIC_CONTENT_RESPONSE)) {
                if (intent.getBooleanExtra(HomeManager.BROADCAST_TOPIC_CONTENT_RESPONSE_STATUS, false)) {
                    ...
                    if (getTopicContentResponse.getData().getSystemName().equalsIgnoreCase(Constants.CONDITIONS_OF_USE))
                        setData();
...
//            tv_title.setText(data.getTitle());
            tv_title2.setText(Html.fromHtml(data.getBody()));

            SpannableString ss = new SpannableString(tv_title2.getText().toString());
...
//r
            ss.setSpan(clickableSpan, 48, 71, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            tv_title2.setText(ss);
            tv_title2.setMovementMethod(LinkMovementMethod.getInstance());
        }
    }
    ...
    private void fetchdata() {
        ...
        String TopicSystemName = Constants.CONDITIONS_OF_USE;
